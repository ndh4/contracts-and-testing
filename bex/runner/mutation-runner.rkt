...
(define recvr1 (make-log-receiver mutation-runner-logger mutation-runner-log-level))
(define recvr2 (make-log-receiver mutate-logger mutate-log-level))

(when print-mutation-runner-logs?
  (void (thread (lambda ()
                  (let loop ()
                    (define v (sync recvr1))
                    (printf "[~a] ~a~n" (vector-ref v 0) (vector-ref v 1))
                    (loop))))))
(when print-mutate-logs?
  (void (thread (lambda ()
                  (let loop ()
                    (define v (sync recvr2))
                    (printf "[~a] ~a~n" (vector-ref v 0) (vector-ref v 1))
                    (loop))))))
